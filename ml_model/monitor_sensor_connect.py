...
class FallDetection:
    ...
    def check_immobility(self):
        ...
        if self.immobility_start_time is None:
            ...
            return "Immobility Detected", False

        return "Normal Activity", False

def get_sensor_data():
    arduino = serial.Serial('COM3', 9600)  # COM port or '/dev/ttyUSB0'
    data = arduino.readline().decode('utf-8').strip()
    arduino.close()

    # Assuming the data format is: "fall_sensor=1;temp=36.5;heart_rate=72"
    sensor_values = dict(item.split('=') for item in data.split(';'))
    
    fall_sensor = int(sensor_values.get('fall_sensor', 0))
    body_temp = float(sensor_values.get('temp', 0))
    heart_rate = int(sensor_values.get('heart_rate', 0))
    
    return fall_sensor, body_temp, heart_rate
...
def main():
    ...
    while True:
        ...
        # Detect movement from webcam feed
        movement_message, fall_detected = detector.detect_movement(frame)

        # Get sensor data
        fall_sensor, body_temp, heart_rate = get_sensor_data()

        # Display sensor data on the frame
        sensor_info = f'Temperature: {body_temp}C, Heart Rate: {heart_rate} BPM'
        cv2.putText(frame, sensor_info, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        if fall_sensor == 1:
            movement_message = "Fall Detected by Sensor!"

        # Display fall detection message
        cv2.putText(frame, movement_message, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255) if fall_detected else (0, 255, 0), 2)
