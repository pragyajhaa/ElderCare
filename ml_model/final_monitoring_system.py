...
class FallDetection:
    ...
    def draw_key_landmarks(self, frame):
        ...
        if len(points) >= 20:
            # Draw connections for the skeleton
            # Torso connections
            cv2.line(frame, points[5], points[11], (0, 255, 0), 2)  # Left shoulder to left hip
            cv2.line(frame, points[6], points[12], (0, 255, 0), 2)  # Right shoulder to right hip
            cv2.line(frame, points[11], points[12], (0, 255, 0), 2)  # Left hip to right hip

            # Upper body connections
            cv2.line(frame, points[5], points[6], (0, 255, 0), 2)  # Left shoulder to right shoulder
            cv2.line(frame, points[5], points[7], (0, 255, 0), 2)  # Left shoulder to left elbow
            cv2.line(frame, points[7], points[9], (0, 255, 0), 2)  # Left elbow to left wrist
            cv2.line(frame, points[6], points[8], (0, 255, 0), 2)  # Right shoulder to right elbow
            cv2.line(frame, points[8], points[10], (0, 255, 0), 2) # Right elbow to right wrist

            # Lower body connections
            cv2.line(frame, points[11], points[13], (0, 255, 0), 2)  # Left hip to left knee
            cv2.line(frame, points[13], points[15], (0, 255, 0), 2)  # Left knee to left ankle
            cv2.line(frame, points[12], points[14], (0, 255, 0), 2)  # Right hip to right knee
            cv2.line(frame, points[14], points[16], (0, 255, 0), 2)  # Right knee to right ankle

            # Feet connections
            cv2.line(frame, points[15], points[17], (0, 255, 0), 2)  # Left ankle to left heel
            cv2.line(frame, points[17], points[19], (0, 255, 0), 2)  # Left heel to left foot index
            cv2.line(frame, points[16], points[18], (0, 255, 0), 2)  # Right ankle to right heel
            cv2.line(frame, points[18], points[20], (0, 255, 0), 2)  # Right heel to right foot index
            
            # Face connections
            cv2.line(frame, points[0], points[1], (0, 255, 0), 2)  # Nose to left eye
            cv2.line(frame, points[0], points[2], (0, 255, 0), 2)  # Nose to right eye
            cv2.line(frame, points[1], points[3], (0, 255, 0), 2)  # Left eye to left ear
            cv2.line(frame, points[2], points[4], (0, 255, 0), 2)  # Right eye to right ear
    ...
    def check_immobility(self):
        ...
        if current_time.hour >= 22 or current_time.hour < 6:
            return "Normal Night Activity", False
        
        if self.immobility_start_time is None:
            self.immobility_start_time = time.time()
            return "Monitoring for Immobility...", False
        
        elapsed_time = time.time() - self.immobility_start_time
        if elapsed_time > self.immobility_threshold:
            return "Fall Detected", True
        return "Monitoring for Immobility...", False
...
def main():
    ...
    while cap.isOpened():
        ...
        movement_type, is_fall = fall_detection.detect_movement(frame)

        # Display movement type on frame
        cv2.putText(frame, movement_type, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
